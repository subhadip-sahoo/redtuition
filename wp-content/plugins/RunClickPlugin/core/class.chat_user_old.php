...
class Chat_user extends Chat_base {
	...
	public function save() {
		 ...
		 */
			...
			$check_ops = get_users('role=g_hangout_op&search=' . $this->email . '&fields=ID' );
			
			if( !empty( $check_admins ) or !empty( $check_ops ) ) {
			
				throw new Exception( __('E-mail address has already been used by an operator. Please enter another one', 'g_hangout' ) );
			...
			if( !g_hangout_name_is_available( $this->name ) )
			...
			$_SESSION['g_hangout']['chat_visitor_ID'] = $wpdb->insert_id;
			
		}
		
		// Prepare user data
		$data = array(
			'visitor_ID'=> ( !empty( $_SESSION['g_hangout']['chat_visitor_ID'] ) ) ? $_SESSION['g_hangout']['chat_visitor_ID'] : null,
			'name'			=> $this->name,
			'email'			=> $this->email,
