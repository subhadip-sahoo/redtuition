...
$baseurl = $upload_dir['baseurl'];
   ...
   $logo_url        =       $baseurl.'/'.str_replace(' ','%20',get_post_meta($g_event_id,'ghanghout_logo',true));
	...
	$msg	=	'<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
		...
		<body style="margin:0px; padding:0px; font: 14px/18px "Source Sans Pro"; background:#F3F2E9; color:#666;">
			...
			<div id="e_wrapper" style="display:block; margin:0px auto; padding:0; width:600px;">		
				...
				.$new_date_for_database.' '.get_post_meta($g_event_id,"hangout_time_zone",true).'<a target="_blank" href="http://www.timeanddate.com/worldclock/fixedtime.html?msg='.str_replace(" ","+",get_the_title($g_event_id)).'&amp;iso='.$hangout_event_date."T".$hangout_event_time.'&amp;p1='.$tz.'"> '.$chktimezone.'</a>
				<br />
...
remove_filter( 'wp_mail_content_type', 'set_html_content_type' );
 if(get_post_meta($g_event_id,"hangout_type",true) == 'Record_hangout'){
	echo $select_date.'ab#ab'.$select_time.'ab#ab'.$email.'ab#ab'.$_POST['locationsearch'].'ab#ab' ;
	}else{
...
$var='Command=Subscriber.Subscribe&ResponseFormat=JSON&JSONPCallBack=true&ListID='.$list_id.'&EmailAddress='.$email.'&IPAddress='.$_SERVER["REMOTE_ADDR"].'';
...
$oiContact = iContactApi::getInstance();
 $add_contact	=	$oiContact->addContact($email, null, null, $name, null, null, null, null, null, null, null,null, null, null);
...
$sendreach = new api();
$sendreach->subscriber_add($list_id,$name,$last_name,$email,$client_ip);
...
$app_name = trim(get_post_meta($g_event_id,'hangout_InfusionSoft_app',true));
 $api_key = trim(get_post_meta($g_event_id,'hangout_InfusionSoft_list_id',true));
