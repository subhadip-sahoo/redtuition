...
class G_Hangout_logs extends WP_List_Table {
    ...
    function get_columns() {
        $columns = array(
            ...
            'title'     	=> __( 'Visitor' , 'g_hangout' ),
            'email'			=> __( 'E-mail', 'g_hangout' ),
            'total_logs'	=> __( 'Total Logs', 'g_hangout' ),
			'last_date'		=> __( 'Last Chat Date', 'g_hangout' )
        );
        return $columns;
    }
    ...
    function column_title($item) {
        ...
        // Build row actions
        $actions = array(
            'edit'      => sprintf('<a href="?page=%s&action=%s&visitor_ID=%s">' . __( 'Chat Logs', 'g_hangout' ) . '</a>', @$_REQUEST['page'], 'edit', @$visitor->ID),
            'delete'    => sprintf('<a href="?page=%s&action=%s&visitor_ID=%s"> ' . __( 'Delete', 'g_hangout' ) . '</a>', @$_REQUEST['page'], 'delete', @$visitor->ID ),
        );
        
		
		
        // Return the title contents
        return sprintf('<div class="g-hangout-list-avatar">%1$s</div><strong>%2$s</strong>%3$s',
	...
	function extra_tablenav( $which ) {
		...
		if( $which == 'top' ) {
			echo '<label class="description">' . __( 'Search' ) . ':</label> <input type="text" name="log_s" id="" placeholder="' . __( 'Search in logs', 'g_hangout' ) . '" value="'. ( ( !empty( $_REQUEST['log_s'] ) ) ?  $_REQUEST['log_s'] : '' ) . '" />';
		}
